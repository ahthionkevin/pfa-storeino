<template>
<div class="flex bg-white">
    <div class="container py-2">
        <div class="flex flex-wrap">
            <div class="w-full md:w-1/2">
                <div class="p-2 border-r border-l h-full">
                    <h3 class=" text-lg font-bold">Experience Moroccofresh - for agriculture.</h3>
                    <p>
                        Whether you are a livestock farmer or a farmer, a contractor or a forester: In the BayWa portal you will find a wide range of animal feed, seeds and crop protection - conventional or ecological - for agriculture. In addition, you will receive detailed information about our diverse advisory services, services, events and topics as well as about us, your BayWa.
                        <br>Discover our diversity - digitally and on site
                    </p>
                </div>
            </div>
            <div class="w-full md:w-1/2">
                <div class="p-2 border-r border-l h-full">
                    <h3 class=" text-lg font-bold">Our payment methods</h3>
                    <div class="flex flex-wrap p-2">
                        <div v-for="item in methods" :key="item.name" class="w-20 h-12 mr-2 flex items-center justify-center border p-2">
                            <img :src="item.image" :alt="item.name" class=" h-full">
                        </div>
                    </div>
                    <h3 class=" text-lg font-bold">Social Media</h3>
                    <div class="flex flex-wrap p-2">
                        <div v-for="item in socialMedia" :key="item.name" class="h-12 mr-2 flex items-center justify-center p-2">
                            <a class="h-full flex" :href="item.url" target="_blank" rel="noopener noreferrer">
                                <img :src="item.image" :alt="item.name" class="">
                            </a>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
</template>
<script>
export default {
    data() {
        return {
            methods: [
                {
                    name: 'Visa',
                    image: 'data:image/png;base64,'
                },
                {
                    name: 'Mastercard',
                    image: 'data:image/png;base64,'
                },
                {
                    name: 'Transfer',
                    image: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRq1oYHa847Gw_Zep09BlvaJPgKeNxt8CAXLg&usqp=CAU'
                },
                {
                    name: 'COD',
                    image: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTnzoLkElehK3UAyy8u_4ESoQdM88_nZjlB3Q&usqp=CAU'
                }
            ],
            socialMedia: [
                {
                    name: 'Facebook',
                    image: 'https://cdn-icons-png.flaticon.com/128/1409/1409943.png',
                    url: '#'
                },
                {
                    name: 'Twitter',
                    image: 'https://cdn-icons-png.flaticon.com/128/1409/1409937.png',
                    url: '#'
                },
                {
                    name: 'Instagram',
                    image: 'https://cdn-icons-png.flaticon.com/128/1409/1409946.png',
                    url: '#'
                },
                {
                    name: 'Youtube',
                    image: 'https://cdn-icons-png.flaticon.com/128/1409/1409936.png',
                    url: '#'
                }
            ]
        }
    },
}
</script>