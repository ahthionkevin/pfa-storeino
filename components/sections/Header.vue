    ...
    <div class="bg-white mb-2 relative">
        <div class="flex p-2 justify-between items-center container">
            <div class="flex h-full justify-start items-center w-1/3">
                ...
                <div v-if="$store.state.settings.store_currencies.length > 1" class="relative mr-1">
                    <div v-if="iconMenu == 'currency'" class="absolute rounded-md flex flex-col top-5 z-10 bg-gray-100">
                        <a class="whitespace-nowrap m-1 p-2 bg-white" v-for="cur of $store.state.settings.store_currencies" :key="cur.code" :href="`?cur=${cur.code}`">{{ cur.name }} ( {{ cur.code }} )</a>
                    </div>
                </div>
                <button @click="iconMenu = iconMenu == 'currency' ? null : 'currency'" v-if="$store.state.settings.store_currencies.length > 1" title="Currency" class="item hidden md:block p-1.5 bg-gray-100 rounded-md hover:bg-gray-200">
                    <span class="text-sm">{{ $store.state.currency.code }}</span>
                </button>
                <div v-if="$store.state.settings.store_languages.length > 1" class="relative mr-1">
                    <div v-if="iconMenu == 'language'" class="absolute rounded-md flex flex-col top-5 z-10 bg-gray-100">
                        <a class="whitespace-nowrap m-1 p-2 bg-white" v-for="lang of $store.state.settings.store_languages" :key="lang.code" :href="`?lang=${lang.code}`">{{ lang.name }} ( {{ lang.code }} )</a>
                    </div>
                </div>
                <button @click="iconMenu = iconMenu == 'language' ? null : 'language'" v-if="$store.state.settings.store_languages.length > 1" title="Language" class="item hidden md:block p-1.5 bg-gray-100 rounded-md hover:bg-gray-200">
            ...
            <div class="icons flex items-center justify-end w-1/3">
                <button v-if="$store.state.settings.sections.header.icons.search" @click="showSearch=true" aria-label="Search button" class="item p-2 bg-gray-100 rounded-md mx-1 hover:bg-gray-200 relative">
                ...
                <router-link v-if="$store.state.settings.sections.header.icons.account" to="/account" title="Account" class="item hidden md:block p-2 bg-gray-100 rounded-md mx-1 hover:bg-gray-200">
                ...
                <router-link v-if="$store.state.settings.sections.header.icons.wishlist" to="/wishlist" title="Wishlist" class="item hidden md:block p-2 bg-gray-100 rounded-md mx-1 hover:bg-gray-200 relative">
                ...
                <router-link v-if="$store.state.settings.sections.header.icons.cart" to="/cart" title="Cart" id="cart-icon" class="item p-2 bg-gray-100 rounded-md mx-1 hover:bg-gray-200 relative">
                    <svg aria-hidden="true" focusable="false" data-prefix="far" data-icon="shopping-cart" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512" class="w-5 h-5"><path fill="currentColor" d="M551.991 64H144.28l-8.726-44.608C133.35 8.128 123.478 0 112 0H12C5.373 0 0 5.373 0 12v24c0 6.627 5.373 12 12 12h80.24l69.594 355.701C150.796 415.201 144 430.802 144 448c0 35.346 28.654 64 64 64s64-28.654 64-64a63.681 63.681 0 0 0-8.583-32h145.167a63.681 63.681 0 0 0-8.583 32c0 35.346 28.654 64 64 64 35.346 0 64-28.654 64-64 0-18.136-7.556-34.496-19.676-46.142l1.035-4.757c3.254-14.96-8.142-29.101-23.452-29.101H203.76l-9.39-48h312.405c11.29 0 21.054-7.869 23.452-18.902l45.216-208C578.695 78.139 567.299 64 551.991 64zM208 472c-13.234 0-24-10.766-24-24s10.766-24 24-24 24 10.766 24 24-10.766 24-24 24zm256 0c-13.234 0-24-10.766-24-24s10.766-24 24-24 24 10.766 24 24-10.766 24-24 24zm23.438-200H184.98l-31.31-160h368.548l-34.78 160z" class=""></path></svg>
